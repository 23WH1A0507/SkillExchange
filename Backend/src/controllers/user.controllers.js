...
export const sendScheduleMeet = asyncHandler(async (req, res) => {
  ...
  const content = `${req.user.name} has requested a meeting at ${time} on ${date}.`;

  let message = await Message.create({ chatId: chat._id, sender: senderId, content });
  ...
  return res.status(201).json(new ApiResponse(201, message, "In-app meeting request sent"));
});
